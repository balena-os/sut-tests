...
const { spawn, exec: Exec } = require('child_process');
...
async function waitSut(release, dutUuid, testId) {
    ...
    await Cloud.sdk.models.device.serviceVar.set(dutUuid, 'browser', `TEST_ID`, testId);
    ...
    return retry(
        async () => {
            const logCatcher = []
            const service_id = (await Cloud.sdk.models.device.getWithServiceDetails(dutUuid)).current_services.sut[0].service_id
