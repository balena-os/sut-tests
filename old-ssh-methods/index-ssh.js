...
async function main(){
    ...
    let result = await new Promise(async (resolve, reject) => {
        let test = spawn('./test.sh', [
            ...
            `${sut}`, 
            '-c',
            `--privileged --network host --env BALENA_API_KEY=${process.env.BALENA_API_KEY} --env WIRED_IF=enp1s0u1u3u4 --env DEVICE_TYPE=raspberrypi3-unipi-neuron --env VERSION=v4.0.7 --env APP=2094765 --env UDEV=1 --env USB_RELAY_CONN=NC -v /lib/modules:/lib/modules -v /run/dbus:/host/run/dbus`
        ], { stdio: 'inherit', timeout: 1000 * 60 * 10 });

        // test.stdout.on('data', (data) => {cd ..
